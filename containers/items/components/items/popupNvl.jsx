...
const Popup_NVL = React.memo(props => {
  ...
  const _HandleSelectedAllVariant = type => {
    if (type == 'main') {
      const uncheckedOptions = optVariantMain.filter(option => !optSelectedVariantMain.some(selectedOpt => selectedOpt.id === option.id));
      ...
      const uncheckedOptions = optVariantSub.filter(option => !optSelectedVariantSub.some(selectedOpt => selectedOpt.id === option.id));
      ...
      <div className='py-4 w-[800px] 2xl:space-y-5 space-y-4'>
        <div className='flex items-center space-x-4 border-[#E7EAEE] border-opacity-70 border-b-[1px]'>
          <button
            ...
            className={`${tab === 0 ? 'text-[#0F4F9E]  border-b-2 border-[#0F4F9E]' : 'hover:text-[#0F4F9E] '} 2xl:text-base text-[15px] px-4 2xl:py-2 py-1 outline-none font-medium`}
          >
            {props.dataLang?.information || 'information'}
          </button>
          <button
            onClick={_HandleSelectTab.bind(this, 1)}
            className={`${tab === 1 ? 'text-[#0F4F9E]  border-b-2 border-[#0F4F9E]' : 'hover:text-[#0F4F9E] '} 2xl:text-base text-[15px] px-4 2xl:py-2 py-1 outline-none font-medium`}
          >
              ...
              {tab === 0 && (
                <div className='grid grid-cols-2 gap-5'>
                  <div className='space-y-2 2xl:space-y-3'>
                    ...
                    <div className='2xl:space-y-1'>
                      <label className='text-[#344054] font-normal 2xl:text-base text-[15px]'>
                        {props.dataLang?.category_material_list_name || 'category_material_list_name'} <span className='text-red-500'>*</span>
                      </label>
                      <input
                        value={name}
                        onChange={_HandleChangeInput.bind(this, 'name')}
                        type='text'
                        placeholder={props.dataLang?.category_material_list_name || 'category_material_list_name'}
                        className={`${
                          errName ? 'border-red-500' : 'focus:border-[#92BFF7] border-[#d0d5dd] '
                        } placeholder:text-slate-300 w-full bg-[#ffffff] rounded text-[#52575E] font-normal  p-2 border outline-none`}
                      />
                      {errName && <label className='text-sm text-red-500'>{props.dataLang?.category_material_list_err_name || 'category_material_list_err_name'}</label>}
                    </div>
                    <div className='2xl:space-y-1'>
                      ...
                      <InPutMoneyFormat
                        ...
                        placeholder={props.dataLang?.category_material_list_cost_price || 'category_material_list_cost_price'}
                        isAllowed={values => {
                      ...
                      <div className='2xl:space-y-1'>
                        ...
                        <div className='relative flex flex-col items-center justify-center'>
                          <InPutNumericFormat
                            ...
                            placeholder={props.dataLang?.category_material_list_expiry_date || 'category_material_list_expiry_date'}
                          ...
                          />
                          <span className='absolute select-none right-2 text-slate-400'>{props.dataLang?.date || 'date'}</span>
                        </div>
                      </div>
                    ) : (
                      ''
                        ...
                        <div className='2xl:space-y-1'>
                          ...
                          <Select
                            ...
                            value={
                              unitChild
                                ? {
                                    label: dataOptUnit?.find(x => x?.value == unitChild)?.label,
                                    value: unitChild,
                                  }
                                : null
                            }
                      ...
                      <div className='flex justify-center'>
                        <div className='relative rounded h-36 w-36 bg-slate-200'>
                          ...
                          <div className='absolute bottom-0 flex flex-col space-y-2 -right-12'>
                            <input onChange={_HandleChangeFileThumb.bind(this)} type='file' id={`upload`} accept='image/*' hidden />
                            <label htmlFor={`upload`} title='Sửa hình' className='flex flex-col items-center justify-center w-8 h-8 rounded-full cursor-pointer bg-slate-100'>
                              <IconEditImg size='17' />
                            </label>
                            <button
                ...
                <div className=''>
                  <div className='grid grid-cols-2 gap-5'>
                    <div className='space-y-3'>
                      ...
                      <div className='flex items-center justify-between'>
                        <h5 className='text-sm text-slate-400'>{props.dataLang?.branch_popup_variant_option || 'branch_popup_variant_option'}</h5>
                        {optVariantMain && (
                          <button onClick={_HandleSelectedAllVariant.bind(this, 'main')} className='text-sm font-medium'>
                            Chọn tất cả
                          </button>
                        )}
                      </div>
                        ...
                        <div className='flex flex-col'>
                          {optVariantMain?.map(e => (
                            <div key={e?.id.toString()} className='flex items-center '>
                              <label className='relative flex items-center p-2 rounded-full cursor-pointer' htmlFor={e.id} data-ripple-dark='true'>
                                <input
                                  type='checkbox'
                              ...
                              <label htmlFor={e.id} className='text-[#344054] font-normal text-sm '>
                                {e.name}
                              </label>
                            </div>
                          ))}
                    ...
                    <div className='space-y-3'>
                      ...
                      <div className='flex items-center justify-between'>
                        <h5 className='text-sm text-slate-400'>{props.dataLang?.branch_popup_variant_option || 'branch_popup_variant_option'}</h5>
                        {optVariantSub && (
                          <button onClick={_HandleSelectedAllVariant.bind(this, 'sub')} className='text-sm font-medium'>
                            Chọn tất cả
                          </button>
                        )}
                      </div>
                  ...
                  {Object.keys(dataTotalVariant).length !== 0 && (
                    <div className='space-y-1'>
                      ...
                      <div className={`${dataTotalVariant[0]?.variation_option_2?.length > 0 ? 'grid-cols-4' : 'grid-cols-4'} grid gap-5 p-1`}>
                        ...
                        <h4 className='text-[15px] font-[300] text-slate-400'>{dataVariantSending[1]?.name}</h4>
                      ...
                      <Customscrollbar className='max-h-[250px] overflow-y-auto overflow-x-hidden'>
                        <div className='space-y-0.5'>
                          {dataTotalVariant?.map((e, index) => (
                            <div
                              className={`${e?.variation_option_2?.length > 0 ? 'grid-cols-4' : 'grid-cols-4'} grid gap-5 items-center bg-slate-50 hover:bg-slate-100 p-1`}
                              ...
                              <div className='flex flex-col items-center justify-center w-full h-full col-span-1'>
                                ...
                                <label htmlFor={`uploadImg+${e?.id}`} className={`${e?.id != null && 'cursor-pointer'} h-14 w-14 flex flex-col justify-center items-center bg-slate-200/50 rounded`}>
                                  {e.image == null ? (
                                    ...
                                    <Image width={64} height={64} src={typeof e.image === 'string' ? e.image : URL.createObjectURL(e.image)} className='object-contain h-14 w-14' />
                                  )}
                                </label>
                              </div>
                                  ...
                                  {e?.variation_option_2?.map(ce => (
                                    <React.Fragment key={ce.id?.toString()}>
                                      ...
                                      <div className='flex justify-center'>
                                        {ce?.isDelete && (
                                          <button
                                            onClick={() => handleQueryId({ id: e.id, status: true, idChild: ce.id })}
        ...
        <div className='flex justify-end space-x-2'>
          <button onClick={_ToggleModal.bind(this, false)} className='px-4 py-2 text-base transition rounded-lg bg-slate-200 hover:opacity-90 hover:scale-105'>
