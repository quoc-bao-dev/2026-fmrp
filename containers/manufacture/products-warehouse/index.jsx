    ...
    <React.Fragment>
      <LayOutTableDynamic
        ...
        titleButton={
          <>
            <h2 className='text-title-section text-[#52575E] capitalize font-medium'>
              ...
              <InfoTooltip
                content='Nhập kho thành phẩm là lịch sử nhập kho được sinh ra từ lệnh sản xuất.'
                iconProps={{
        ...
        fillterTab={
          <>
            {dataFillterbar &&
              dataFillterbar?.map(e => {
                return (
                  <TabFilter backgroundColor='#e2f0fe' dataLang={dataLang} key={e.id} onClick={_HandleSelectTab.bind(this, e.id)} total={e.count} active={e.id}>
              ...
              <div className='col-span-1 xl:col-span-2 lg:col-span-2'>
                <div className='flex items-center justify-end gap-2'>
                  ...
                  {role == true || checkExport ? (
                    <div className={``}>
                      {data?.rResult?.length > 0 && <ExcelFileComponent dataLang={dataLang} filename={'Danh sách nhập kho thành phẩm'} title='DSNKTP' multiDataSet={multiDataSet} />}
                    </div>
                  ) : (
                    <button
                      ...
                      <RowTable gridCols={11} key={e.id.toString()}>
                        ...
                        <RowItemTable colSpan={1} textAlign={'left'}>
                          <div className='flex flex-col gap-1'>
                            ...
                            <span className='responsive-text-xxs text-[#0375F3]'>{e?.reference_no_detail}</span>
                          </div>
                        </RowItemTable>
                        <RowItemTable colSpan={1} textAlign={'left'}>
                          <LinkWarehouse disbleClick={true} colSpan={1} warehouse_id={e?.warehouse_id} warehouse_name={e?.warehouse_name} />
                        </RowItemTable>
                        ...
                        <RowItemTable colSpan={1} className={'flex items-center justify-start gap-2'}>
                          <CustomAvatar data={e} fullName={e?.staff_create?.full_name} profileImage={e?.staff_create?.profile_image} />
                        </RowItemTable>
        ...
        total={
          <ContainerTotal className='!grid-cols-22'>
            <RowItemTable colSpan={3.5} textAlign={'center'} className='p-2'></RowItemTable>
      ...
      />
      {isState.dataExport?.length > 0 && <PopupStatus type='productsWarehouse' className='hidden' dataExport={isState.dataExport} dataLang={dataLang} />}
      <PopupConfim
        dataLang={dataLang}
        type='warning'
