...
const Header = props => {
  ...
  const countActiveFilters = filters => {
    ...
    Object.entries(filters).forEach(([key, value]) => {
      ...
      else if (key === 'valueDate' && (value?.startDate != null || value?.endDate != null)) {
        count++;
      }
    });
  ...
  const triggerFilterAll = (
    <button
      className={`${
        stateFilterDropdown?.open || activeFilterCount > 1
    ...
    >
      ...
      <span className={`${stateFilterDropdown?.open || activeFilterCount > 1 ? 'text-[#0F4F9E]' : 'text-[#3A3E4C] group-hover:text-[#0F4F9E]'} text-nowrap 3xl:text-base text-sm custom-transition`}>
      ...
      <span className='3xl:size-4 size-3.5 shrink-0'>
        <CaretDownIcon className={`${stateFilterDropdown?.open || activeFilterCount > 1 ? 'rotate-180' : 'rotate-0'} w-full h-full custom-transition`} />
      </span>
    </button>
  );
      ...
      <div className='flex items-center justify-between'>
        <h2 className='text-title-section text-[#52575E] capitalize font-medium'>
          ...
          <InfoTooltip
            content={
              <p>
                Kế hoạch sản xuất là bảng tổng hợp nhu cầu sản phẩm từ <span className='font-bold'>Đơn hàng bán & Kế hoạch nội bộ</span>, giúp bộ phận sản xuất tổng hợp tổng sản lượng và lên lệnh sản
                xuất từ đây.
              </p>
            }
            iconProps={{
            ...
            onClick={() => {
              if (role || checkAdd) {
                if (isCheck) {
                  ...
                  showToat('error', router.query?.tab == 'plan' ? 'Vui lòng chọn ít nhất một KHNB' : 'Vui lòng chọn ít nhất một đơn hàng');
                }
              } else {
                showToat('error', WARNING_STATUS_ROLE);
              }
            }}
            className='responsive-text-sm 3xl:py-3 3xl:px-4 py-2 px-3 bg-blue-fmrp hover:bg-blue-fmrp text-white rounded-lg btn-animation flex items-center gap-x-2 text-nowrap whitespace-nowrap'
          >
