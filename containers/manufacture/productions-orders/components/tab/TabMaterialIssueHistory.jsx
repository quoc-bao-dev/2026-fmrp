...
const TabMaterialIssueHistory = ({ dataLang, ...props }) => {
  ...
  const flagMaterialIssueHistory = useMemo(() => (dataMaterialIssueHistory ? dataMaterialIssueHistory?.pages?.flatMap(page => page.rResult) : []), [dataMaterialIssueHistory]);
    ...
    <div className='flex flex-col 3xl:gap-6 gap-4'>
      <div className='flex items-center justify-end gap-3'>
        ...
        {flagMaterialIssueHistory?.length > 0 && (
          <ExcelFileComponent dataLang={dataLang} filename={'Danh sách dữ liệu lịch sử xuất NVL/BTP'} multiDataSet={multiDataSet} title='DSDLLS NVL/BTP'>
            <ButtonAnimationNew
              icon={
                <div className='3xl:size-5 size-4'>
        ...
        <div className='col-span-16 grid grid-cols-16 min-h-[240px]'>
          {isLoadingMaterialReturn || isLoadingTable ? (
            ...
            flagMaterialIssueHistory?.map((product, index) => (
              ...
              >
                <h4 className='col-span-1 flex items-center justify-center size-full text-center text-[#141522] font-semibold text-sm-default uppercase px-1'>{index + 1 ?? '-'}</h4>

                <h4 className='col-span-3 flex flex-col justify-center size-full gap-1 text-[#141522] px-1'>
                  <div className='text-sm-default font-semibold'>{product?.date ? formatMoment(product?.date, FORMAT_MOMENT.DATE_SLASH_LONG) : '-'}</div>

                  <div className='text-xs-default font-normal'>{product?.date ? formatMoment(product?.date, FORMAT_MOMENT.TIME_LONG) : ''}</div>
                </h4>

                <h4 className='col-span-3 flex items-center size-full text-start text-[#141522] font-semibold text-sm-default px-1'>{product?.code ?? '-'}</h4>
                ...
                <h4 className='col-span-5 text-[#344054] font-normal flex items-center justify-center size-full gap-2 px-1'>
                  <div className='flex items-start justify-start w-full gap-2'>
                    ...
                    <div className='flex flex-col 3xl:gap-1 gap-0.5'>
                      <p className={`font-semibold text-sm-default text-[#141522] group-hover:text-[#0F4F9E]`}>{product?.item_name ?? ''}</p>
                        ...
                        {product?.type_item === 'material' && Array.isArray(warehousePropertyLabels) && warehousePropertyLabels.length > 0 && (
                          <div className='flex flex-wrap items-center gap-2'>
                            {warehousePropertyLabels.map(({ key, label }) => {
                              ...
                              const value = product?.[key] ?? product?.item?.[key];
