...
const MainTable = ({ dataLang }) => {
  ...
  }, [isValue.search, isValue.dateStart, isValue.dateEnd, isValue.valueOrder, isValue.valuePlan, isValue.valueBr, isMounted, isValue.valueDate]);
            ...
            <Customscrollbar className='3xl:h-[65vh] xxl:h-[54.5vh] 2xl:h-[56vh] xl:h-[56vh] lg:h-[55vh] h-[35vh] overflow-y-auto'>
              {dataTable.productionOrdersList.map((e, eIndex) => (
                <div
                  ...
                  className={`py-2 pl-2 pr-3 ${e.id == isParentId && 'bg-[#F0F7FF]'} hover:bg-[#F0F7FF] cursor-pointer transition-all ease-linear ${
                    dataTable.length - 1 == eIndex ? 'border-b-none' : 'border-b'
                  } `}
                >
                  <div className='flex justify-between'>
                    <div className='flex flex-col gap-1'>
                      <h1 className='3xl:text-base xxl:text-base 2xl:text-sm xl:text-xs lg:text-xs text-sm font-medium text-[#0F4F9E]'>{e.title}</h1>
                      <h3 className='text-[#667085] font-normal text-[11px]'>
                        {dataLang?.materials_planning_create_on || 'materials_planning_create_on'} <span className='text-[#141522] font-medium 3xl:text-xs text-[11px]'>{e.time}</span>
                      </h3>
                      {/* <TagBranch className="w-fit">{e?.nameBranch}</TagBranch> */}
                      <p className='responsive-text-sm font-semibold text-wrap py-1'>{e?.nameBranch}</p>
                    </div>
                  </div>
                  {e.id == isParentId && (
                    <div className='flex flex-col gap-2 mt-1'>
                      <div className='flex items-center gap-1'>
                        <h3 className=' text-[#52575E] font-normal 3xl:text-sm text-xs'>{dataLang?.materials_planning_foloww_up || 'materials_planning_foloww_up'} :</h3>
                        <div className='flex items-center gap-1'>
                          {e.followUp.map(i => (
                            <React.Fragment key={i.id}>
                              <h2 className='text-[#191D23] font-medium 3xl:text-sm text-xs'>{i.nameFollow}</h2>
                            </React.Fragment>
                          ))}
                        </div>
                      </div>
                  ...
                  {dataTable.listDataRight?.title ? (
                    <div className='flex items-center'>
                      ...
                      {dataTable.listDataRight?.referenceNoPo && (
                        <span>
                          / <TagColorOrange onClick={() => {}} name={dataTable.listDataRight?.referenceNoPo} className={'relative !text-[11px] top-0 3xl:!py-1 !py-1 cursor-pointer select-none'} />
                        </span>
                      )}
                    </div>
                  ) : (
                ...
                {arrButton.map(e => (
                  <Zoom key={e.id} whileHover={{ scale: 1.05 }} whileTap={{ scale: 1.08 }} className='w-fit'>
                    {(e.id == 3 && (
                      ...
                      (e.id == 2 && <PopupPurchaseBeta id={e.id} queryValue={queryValue} fetchDataTable={fetchDataTable} dataLang={dataLang} title={e.name} dataTable={dataTable} icon={e.icon} />)}
                  </Zoom>
                ))}
              </div>
            </div>
                  ...
                  {listTab.map(e => (
                    <button
                      ...
                      className={`hover:bg-[#F7FBFF] ${
                        dataTable.isTab == e.type ? 'border-[#0F4F9E] border-b bg-[#F7FBFF]' : 'border-b border-transparent'
