        ...
        titleButton={
          <>
            <h2 className='text-title-section text-[#52575E] capitalize font-medium'>
              ...
              <InfoTooltip
                content='Kế hoạch nội bộ là kế hoạch sản xuất nội bộ của công ty, được lập dựa trên nhu cầu trữ hàng và các yếu tố khác (làm bù đơn, làm bù hàng lỗi...).'
                iconProps={{
          ...
          <div className='flex flex-col h-full'>
            <div className='w-full items-center flex justify-between gap-2'>
              <div className='flex gap-3 items-center w-full'>
                <SearchComponent dataLang={dataLang} colSpan={3} onChange={handleOnChangeKeySearch.bind(this)} />
                <SelectComponent
                  colSpan={3}
                  options={[
                    {
                ...
                />
                <DateToDateComponent className='col-span-3' value={idFillter.valueDate} onChange={onChangeFilter('valueDate')} colSpan={3} />
              </div>
              <div className='flex items-center justify-end gap-2'>
                <OnResetData sOnFetching={() => {}} onClick={() => refetch()} />
                {role == true || checkExport ? (
                  <div className={``}>{data?.rResult?.length > 0 && <ExcelFileComponent dataLang={dataLang} filename={'Danh sách kế hoạch nội bộ'} multiDataSet={multiDataSet} title='DSKHNB' />}</div>
                ) : (
                  <button
                    ...
                    <div className='divide-y divide-slate-200 h-[100%]'>
                      {data?.rResult?.map((e, index) => (
                        <RowTable key={e.id.toString()} gridCols={20}>
                          ...
                          <RowItemTable colSpan={2} textAlign={'left'} className='flex items-center space-x-1'>
                            <CustomAvatar classNameAvatar='max-w-[26%] w-[26%]' profileImage={e?.created_by_profile_image} fullName={e?.created_by_full_name} />
                          </RowItemTable>
                          <RowItemTable colSpan={2}>
                            {/* <TagBranch className="w-fit">
                                                            {e?.name_branch}
                                                        </TagBranch> */}
                            <span className='flex flex-wrap items-center justify-start gap-2'>{e?.name_branch}</span>
                          </RowItemTable>
