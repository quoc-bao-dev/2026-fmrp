...
const Warehouse = props => {
  ...
  // Tính toán cấu trúc grid cho header/body dựa trên cấu hình
  const isSerialEnabled = dataProductSerial?.is_enable === '1';
  const isExpiryEnabled = dataMaterialExpiry?.is_enable === '1' || dataProductExpiry?.is_enable === '1';

  // Base: 2 (sản phẩm) + 1 (vị trí) + 1 (biến thể chính) + 1 (biến thể phụ) + 1 (số lượng) + 1 (giá trị) = 7
  const baseHeaderCols = 7;
  const attributeHeaderCols = showWarehouseAttributesColumn ? 2 : 0; // cột Thuộc tính span 2
  const serialHeaderCols = isSerialEnabled ? 1 : 0;
  const expiryHeaderCols = isExpiryEnabled ? 2 : 0; // Lot + HSD

  const headerGridCols = baseHeaderCols + attributeHeaderCols + serialHeaderCols + expiryHeaderCols;

  // Map headerGridCols -> class grid tương ứng (Tailwind cần chuỗi tĩnh)
  const rowGridColsClassMap = {
    7: 'grid-cols-14',
    8: 'grid-cols-16',
    9: 'grid-cols-18',
    10: 'grid-cols-20',
  ...
  // Phần detail sau cột sản phẩm (colSpan = 2) chiếm headerGridCols - 2 cột logic
  const detailWrapperColSpanClassMap = {
    7: 'col-span-10', // (7 - 2) * 2 = 10
    8: 'col-span-12',
    9: 'col-span-14',
    10: 'col-span-16',
    11: 'col-span-18',
  ...
  const detailInnerGridColsClassMap = {
    7: 'grid-cols-10',
    8: 'grid-cols-12',
    9: 'grid-cols-14',
    10: 'grid-cols-16',
  ...
  const rowGridClass = rowGridColsClassMap[headerGridCols] || 'grid-cols-16';
  const detailWrapperColSpanClass = detailWrapperColSpanClassMap[headerGridCols] || 'col-span-12';
  const detailInnerGridColsClass = detailInnerGridColsClassMap[headerGridCols] || 'grid-cols-12';
  ...
  return (
    <React.Fragment>
      <LayOutTableDynamic
        ...
        titleButton={
          <>
            <h2 className='text-title-section text-[#52575E] capitalize font-medium'>{dataLang?.Warehouse_title || 'Warehouse_title'}</h2>
              ...
              <div className='col-span-1 xl:col-span-2 lg:col-span-2'>
                <div className='flex items-center justify-end gap-2 space-x-2'>
                  ...
                  {role == true || checkExport ? (
                    <div className={``}>{dataWarehouse?.rResult?.length > 0 && <ExcelFileComponent dataLang={dataLang} filename='Danh sách kho' title='Dsk' multiDataSet={multiDataSet} />}</div>
                  ) : (
                    <button
            ...
            <div className='grid min-h-0 grid-cols-10 '>
                        ...
                        <div className='relative grid grid-cols-12 mt-1'>
                          <li
                            className={` col-span-12 ${
                              isState.idWarehouse === item.id ? 'bg-[#3276FA] text-white' : ''
                          ...
                          >
                            <div className='flex xl:w-[90%] xl:max-w-[90%] w-[85%] max-w-[85%] items-center gap-2'>
                              ...
                              <div className='flex flex-col items-start w-full'>
                                <div className='w-[95%] max-w-[95%] 3xl:text-base xl:text-sm text-xs '>{item.name}</div>
                                {item.is_system == 1 && (
                    ...
                    >
                      {isState.idWarehouse === item.id && item.is_system == 0 && (
                        <div className='flex items-center gap-2 px-2 py-[5px] ml-2 bg-gray-200 rounded-md 3xl:py-3'>
                          ...
                          <BtnAction onRefresh={refetch.bind(this)} onRefreshGroup={refetchWarehouseDetail.bind(this)} dataLang={dataLang} id={item?.id} type='warehouse' />
                        </div>
                      )}
                    </PopupParent>
                  ))}
              ...
              <Customscrollbar className='h-full col-span-8 overflow-y-auto'>
                <div className={`w-full`}>
                  {/* header table */}
                  <HeaderTable gridCols={headerGridCols}>
                  ...
                  {isFetching || isFetchingWarehouseDetail ? (
                    ...
                    <div className='w-full h-full'>
                      {data?.rResult &&
                        data?.rResult?.map(e => (
                          <div className={`${rowGridClass} grid hover:bg-slate-50 px-2`}>
                                  ...
                                  <div className='flex w-full gap-2 px-2 3xl:gap-3'>
                                    <div className='3xl:w-[25%] 3xl:max-w-[25%] xl:w-[35%] xl:max-w-[35%] w-[30%] max-w-[30%] flex items-center'>
                                      <Image
                                        ...
                                        width={70}
                                        height={70}
                                        className='xxl:w-[50px] xxl:min-w-[50px] xxl:h-[50px] xl:w-[40px]- xl:min-w-[40px] xl:h-[40px] w-[40px] min-w-[40px] h-[40px] rounded object-contain'
                                      />
                                    </div>
                                    <div className='3xl:w-[75%] 3xl:max-w-[75%] xl:w-[65%] xl:max-w-[65%] w-[70%] max-w-[70%] flex flex-col 3xl:gap-2 gap-1'>
                                      <h6 className='w-full font-semibold text-left responsive-text-base text-zinc-600'>{e.item_name == null ? '-' : e.item_name}</h6>

                                      <h6 className='w-full font-medium text-left responsive-text-sm text-zinc-500 break-words'>{e.item_code == null ? '-' : e.item_code}</h6>

                                      {(e?.stage_id > 0 && e?.type_products == 'products' && <TagColorProduct dataKey={7} lang={false} dataLang={dataLang} name={dataLang?.product_dd} />) ||
                                        (e?.stage_id > 0 && e?.type_products == 'semi_products' && (
                                          <TagColorProduct dataKey={7} lang={false} dataLang={dataLang} name={dataLang?.semi_products_dd} />
                                        )) ||
                                        (e?.stage_id > 0 && e?.type_products == 'semi_products_outside' && (
                                          <TagColorProduct dataKey={7} lang={false} dataLang={dataLang} name={dataLang?.semi_products_outside_dd} />
                                        )) || <TagColorProduct dataKey={e?.item_type === 'product' ? 0 : 1} dataLang={dataLang} name={e?.item_type} />}
                                    </div>
                                  </div>
                                ) : (
                                  <div className='flex w-full gap-2 px-2 3xl:gap-3'>
                                    <div className='3xl:w-[25%] 3xl:max-w-[25%] xl:w-[35%] xl:max-w-[35%] w-[30%] max-w-[30%] flex items-center'>
                                      <Image
                                        ...
                                        width={70}
                                        height={70}
                                        className='xxl:w-[50px] xxl:min-w-[50px] xxl:h-[50px] xl:w-[50px] xl:min-w-[50px] xl:h-[50px] w-[40px] min-w-[40px] h-[40px] rounded-lg object-cover'
                                      />
                                    </div>
                                    <div className='3xl:w-[75%] 3xl:max-w-[75%] xl:w-[65%] xl:max-w-[65%] w-[70%] max-w-[70%] flex flex-col 3xl:gap-2 gap-1'>
                                      <h6 className='w-full font-semibold text-left responsive-text-base text-zinc-600 '>{e.item_name == null ? '-' : e.item_name}</h6>
                            ...
                            <div className={`grid ${detailWrapperColSpanClass}`}>
                              {e?.detail.map(item => (
                                <div className={`grid ${detailInnerGridColsClass}`}>
                                  <RowItemTable
                                    // textSize={"text-[12px]"}
                                    colSpan={1}
                                  ...
                                  >
                                    {item?.quantity ? (
                                      <>
                                        <span className=''>{formatNumber(+item?.quantity)}</span>/<span className='relative pt-1 pl-0.5 capitalize top-1'>{e?.unit}</span>
                                      </>
                                    ) : (
                                      '-'
                                    )}
