...
import { useMainstreamGoodsPrices } from '@/managers/api/mainstream-goods/useMainstreamGoodsPrices';
...
const PurchaseImportForm = props => {
  ...
  const { fetchMainstreamGoodsPricesAsync } = useMainstreamGoodsPrices();
      ...
      sListData(
        rResult?.items.map(e => ({
          ...
          item: {
            ...
            value: e.item?.purchase_order_item_id === '0' ? e.item?.id : e.item?.id + '__' + e.item?.purchase_order_item_id,
            // value: e.item?.id,
          },
  ...
  const resetValue = () => {
    if (isKeyState?.type === 'supplier') {
      ...
      const itemVariationIds = listData?.map(item => item?.item?.e?.id).filter(id => id !== undefined && id !== null);

      if (itemVariationIds?.length > 0) {
        (async () => {
          try {
            const response = await fetchMainstreamGoodsPricesAsync({
              supplier_id: isKeyState.value.value,
              item_variation_ids: itemVariationIds,
            });

            // Cập nhật lại giá từng item trong listData dựa theo key_item và price từ response
            const mainstreamGoods = response?.data?.mainstream_goods || {};
            if (mainstreamGoods && typeof mainstreamGoods === 'object') {
              const updatedListData = listData.map(item => {
                const keyItem = item?.item?.e?.id;
                const priceInfo = mainstreamGoods[keyItem];

                // Nếu không có giá trong response, set giá về 0
                const rawPrice = priceInfo?.price;
                const parsedPrice = Number(rawPrice);
                const newPrice = !rawPrice || Number.isNaN(parsedPrice) ? 0 : parsedPrice;

                const newChild = item?.child?.map(childItem => ({
                  ...childItem,
                  price: newPrice,
                }));
              ...
              sListData(updatedListData);
            }
          } catch (error) {
            console.error('Mainstream goods prices error (supplier confirm):', error);
          }
        })();
      }
    }
  ...
  const _HandleChangeInput = (type, value) => {
    if (type == 'code') {
      ...
      if (value?.length === 0) {
        ...
        const updatedListData = listData.filter(existingItem => value.some(selectedItem => selectedItem.value === existingItem.item.value));
        // Thêm các mặt hàng mới được chọn
        const newSelectedItems = value.filter(selectedItem => !updatedListData.some(existingItem => existingItem.item.value === selectedItem.value));
        const newData = newSelectedItems.map((e, index) => _DataValueItem(e, updatedListData.length + index));
  ...
  const _HandleSubmit = e => {
    ...
    const hasNullWarehouse = listData.some(item =>
      item.child?.some(childItem => {
        ...
        return childItem.warehouse === null || (id && (childItem.warehouse?.label === null || childItem.warehouse?.warehouse_name === null));
      })
    );
    ...
    const checkNumber = listData.some(item =>
      item.child?.some(childItem => childItem.price === '' || childItem.price === null || childItem.amount === '' || childItem.amount === 0 || childItem.amount === null)
    );
    if (
      idSupplier == null ||
      idBranch == null ||
  ...
  const caculateAmount = option => {
    ...
    const totalTax = option?.reduce((accumulator, item) => {
      const childTotal = item.child?.reduce((childAccumulator, childItem) => {
        const product = Number(childItem?.price * (1 - childItem?.discount / 100)) * (isNaN(childItem?.tax?.tax_rate) ? 0 : Number(childItem?.tax?.tax_rate) / 100) * Number(childItem?.amount);
        return childAccumulator + product;
      }, 0);
      return accumulator + childTotal;
    }, 0);
    ...
    const totalAmount = option?.reduce((accumulator, item) => {
      const childTotal = item.child?.reduce((childAccumulator, childItem) => {
        const product = Number(childItem?.price * (1 - childItem?.discount / 100 || 0)) * (1 + Number(childItem?.tax?.tax_rate || 0) / 100) * Number(childItem?.amount);
        return childAccumulator + product;
      }, 0);
      return accumulator + childTotal;
    }, 0);
        ...
        const newChild = e?.child?.map(ce => {
          if (ce?.id === childId) {
            ...
            //     return { ...ce, serial: value?.target.value };
              ...
              const isValueExists = e.child.some(otherCe => otherCe[type] === value?.target.value && otherCe.id !== childId);

              if (isValueExists) {
                handleQuantityError(`Giá trị ${type} đã tồn tại`);
                return ce; // Trả về giá trị ban đầu nếu đã tồn tại
              } else {
                const otherElements = listData.filter(otherE => otherE.id !== parentId && otherE.child.some(otherCe => otherCe[type] === value?.target.value));
  ...
  const selectItemsLabel = (option, isOnTable = false) => (
    <div className={`flex items-start py-2 font-deca ${!isOnTable && 'cursor-pointer'}`}>
      <div className={`flex ${isOnTable && 'xl:flex-row flex-col'} ${isOnTable ? 'xl:items-center items-start' : 'items-center'} gap-3`}>
        <img src={option.e?.images ?? '/icon/noimagelogo.png'} alt={option?.e.name} className={`${isOnTable ? 'size-12' : 'xl:size-16 size-12'} object-cover rounded-md`} />
        <div className='flex flex-col gap-1 3xl:text-[10px] text-[9px] font-normal overflow-hidden w-full'>
          ...
          <div className='flex flex-wrap items-center text-neutral-03'>
            {isOnTable && `ĐVT: ${option.e?.unit_name} - `} {dataLang[option.e?.text_type]} - {dataLang?.purchase_survive || 'purchase_survive'}:{' '}
            {option.e?.qty_warehouse ? formatNumber(option.e?.qty_warehouse) : '0'}
          </div>
          {!isOnTable && option?.e?.id_plan > 0 && <TagColorProduct lang={false} dataKey={1} name={option?.e?.reference_no_plan} />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <LayoutOrderManagement
          ...
          {listData.length === 0 ? (
            ...
            <div>
              <div className='grid grid-cols-13 items-center gap-3 sticky top-0 z-10 py-2 border-b border-gray-100'>
                <TableHeader className='col-span-2 text-left'>{dataLang?.import_from_items || 'import_from_items'}</TableHeader>
                  ...
                  >
                    <Dropdown
                      overlay={
                        <div className='border px-4 py-5 shadow-lg bg-white rounded-lg'>
                          <p className='3xl:text-base 2xl:text-sm text-[12px] font-normal font-deca text-secondary-color-text mb-2'>Chọn nhanh kho - Vị trí</p>
                          <SelectCustomLabel
                            dataLang={dataLang}
                    ...
                    >
                      <div className='inline-flex items-center justify-between cursor-pointer group'>
                        <TableHeader className='text-start group-hover:text-neutral-05'>{dataLang?.PDF_house || 'PDF_house'}</TableHeader>
                        <ArrowDown2 size={16} className='text-neutral-02 font-medium group-hover:text-neutral-05' />
                      </div>
                    </Dropdown>

                    {dataProductSerial?.is_enable === '1' && <TableHeader className='col-span-1 text-center'>{'Serial'}</TableHeader>}
                    {(dataMaterialExpiry?.is_enable === '1' || dataProductExpiry?.is_enable === '1') && (
                      <>
                        <TableHeader className='col-span-1 text-center'>{'Lot'}</TableHeader>
                        <TableHeader className='col-span-1 text-center'>{props.dataLang?.warehouses_detail_date || 'Date'}</TableHeader>
                      </>
                    )}
                    <TableHeader className='col-span-1 text-center'>{dataLang?.import_from_quantity || 'import_from_quantity'}</TableHeader>
                    <TableHeader className='col-span-1 text-center'>{dataLang?.import_from_unit_price || 'import_from_unit_price'}</TableHeader>
                    ...
                    <DropdownTax taxOptions={taxOptions} totalTax={tax} onChange={_HandleChangeInput.bind(this, 'tax')} dataLang={dataLang} />

                    <TableHeader className='col-span-1 text-right'>{dataLang?.import_into_money || 'import_into_money'}</TableHeader>
                  </div>
                </div>
              </div>
                  ...
                  {isFetching ? (
                    ...
                    <>
                      {listData?.map((e, index) => {
                        ...
                        return (
                          <div key={e?.id?.toString()} className={`grid items-start gap-3 py-2 grid-cols-13 ${isLast ? '' : 'border-b border-[#F3F3F4]'}`}>
                            {/* Mặt hàng */}
                                  ...
                                  <React.Fragment key={ce?.id?.toString()}>
                                    ...
                                    <div className={`flex flex-col items-center justify-center`}>
                                      {ce?.id_plan > 0 && ce?.reference_no_plan ? (
                                        <TagColorProduct lang={false} dataKey={6} name={`Vị trí theo ${ce?.reference_no_plan}`} className='3xl:!text-[13px] xl:!text-[10px] !text-[9px]' />
                                      ) : (
                                        <div className='flex flex-col w-full'>
                                          <SelectCustomLabel
                                            ...
                                            isError={(errWarehouse && ce?.warehouse == null) || (errWarehouse && (ce?.warehouse?.label == null || ce?.warehouse?.warehouse_name == null))}
                                            isVisibleLotDate={false}
                                          />
                                          {((errWarehouse && ce?.warehouse == null) || (errWarehouse && (ce?.warehouse?.label == null || ce?.warehouse?.warehouse_name == null))) && (
                                    ...
                                    <div
                                      className={`flex items-center justify-center h-8 2xl:h-10 3xl:p-2 xl:p-[2px] p-[1px] border rounded-3xl ${
                                        ce?.amount === '' || ce?.amount === null || ce?.amount === 0 ? 'border-red-500' : 'focus:border-brand-color hover:border-brand-color border-neutral-N400'
                                      }`}
                                    >
                                      <button
                                    ...
                                    <div
                                      className={`flex items-center justify-center h-8 2xl:h-10 py-1 px-2 2xl:px-3 rounded-lg border ${
                                        ce?.price === '' || ce?.price === null ? 'border-red-500' : 'focus:border-brand-color hover:border-brand-color border-neutral-N400'
                                      }`}
                                    >
                                      <InPutMoneyFormat
                                    ...
                                    <div className='flex items-center'>
                                      <SelectCustomLabel
                                        ...
                                        renderOption={(option, isLabel) => (
                                          <div className={`flex items-center justify-start gap-1 responsive-text-sm ${isLabel ? 'py-1 2xl:py-2' : ''}`}>
                                            <h2 className=''>{option?.label}</h2>
                                            {option?.tax_rate !== '0' && option?.tax_rate !== '5' && <h2>{option?.tax_rate === '20' ? `(${option?.tax_rate}%)` : `${option?.tax_rate}%`}</h2>}
                                          </div>
                                        )}
                                      />
                                    </div>
                                    ...
                                    <ItemTotalAndDelete
                                      total={formatMoney(ce?.price * (1 - Number(ce?.discount || 0) / 100) * (1 + Number(ce?.tax?.tax_rate ? ce?.tax?.tax_rate : 0) / 100) * Number(ce?.amount))}
      ...
      note={<TextareaNote label={dataLang?.purchase_order_note || 'purchase_order_note'} value={note} onChange={_HandleChangeInput.bind(this, 'note')} />}
      isTotalMoney={total}
