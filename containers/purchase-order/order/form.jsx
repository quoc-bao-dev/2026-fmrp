...
const OrderForm = props => {
  ...
  const _HandleChangeInputOption = (id, type, index3, value) => {
    ...
    if (type == 'items') {
      if (sortedArr[index].items) {
        ...
        sortedArr[index].total = Number(sortedArr[index].affterDiscount) * (1 + Number(0) / 100) * Number(sortedArr[index].quantity);
      } else {
        let moneyClient = 0;
        ...
        if (newData.tax?.e?.tax_rate == undefined) {
          ...
          const money = Number(newData.affterDiscount) * (1 + Number(newData.tax?.e?.tax_rate) / 100) * Number(newData.quantity);
          newData.total = Number(money.toFixed(2));
        }
      ...
      if (sortedArr[index].tax?.tax_rate == undefined) {
        ...
        const money = Number(sortedArr[index].affterDiscount) * (1 + Number(sortedArr[index].tax?.tax_rate) / 100) * Number(sortedArr[index].quantity);
        sortedArr[index].total = Number(money.toFixed(2));
      }
  ...
  return (
    <React.Fragment>
      <LayoutForm
        title={id ? dataLang?.purchase_order_edit_order || 'purchase_order_edit_order' : dataLang?.purchase_order_add_order || 'purchase_order_add_order'}
        ...
        total={
          <div className='flex flex-col gap-3 justify-between text-right '>
            ...
            <div className='flex justify-between '>
              <div className='font-normal'>
                <h3>{dataLang?.purchase_order_detail_money_after_discount || 'purchase_order_detail_money_after_discount'}</h3>
              </div>
              <div className='font-normal'>
