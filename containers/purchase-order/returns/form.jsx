...
const PurchaseReturnsForm = props => {
          ...
          if (ce?.id === childId) {
            if (type === 'amount') {
              ...
              if (ce?.id === childId && totalSoLuong == qtyHouse) {
                ...
                isShow('error', `Tổng số lượng chỉ được bé hơn hoặc bằng ${formatNumber(Number(ce?.kho?.qty))} số lượng còn lại`);

                return { ...ce };
              } else if (ce?.id === childId && totalSoLuong > Number(ce?.kho?.qty)) {
                isShow('error', `Tổng số lượng chỉ được bé hơn hoặc bằng ${formatNumber(Number(ce?.kho?.qty))} số lượng tồn`);
  ...
  const breadcrumbItems = [
    ...
    {
      label: `${id ? dataLang?.returns_title_edit || 'returns_title_edit' : dataLang?.returns_title_child || 'returns_title_child'}`,
    },
  ];
  ...
  const selectItemsLabel = (option, isOnTable = false) => (
    <div className={`flex items-start py-2 font-deca ${!isOnTable && 'cursor-pointer'}`}>
      <div className={`flex ${isOnTable && 'xl:flex-row flex-col'} ${isOnTable ? 'xl:items-center items-start' : 'items-center'} gap-3`}>
        <img src={option.e?.images ?? '/icon/noimagelogo.png'} alt={option?.e?.name || 'Product image'} className={`${isOnTable ? 'size-12' : 'xl:size-16 size-12'} object-cover rounded-md`} />
        <div className='flex flex-col gap-1 3xl:text-[10px] text-[9px] font-normal overflow-hidden w-full'>
          <h3 className={`font-semibold responsive-text-sm ${isOnTable ? 'text-brand-color' : 'text-black'}`}>{option.e?.name || option.e?.item_name}</h3>
          ...
          <h5 className='flex flex-wrap items-center text-neutral-03'>
            {option.e?.import_code} - {`(ĐGSCK: ${formatMoney(option.e?.price_after_discount)}) -`} {dataLang[option.e?.text_type]}
          </h5>
          ...
          <div className='flex items-center gap-2 italic text-neutral-03'>
            ...
            {dataMaterialExpiry.is_enable === '1' || dataProductExpiry.is_enable === '1' ? (
              <>
                ...
                <div>Date: {option.e?.expiration_date ? formatMoment(option.e?.expiration_date, FORMAT_MOMENT.DATE_SLASH_LONG) : '-'}</div>
              </>
            ) : (
              ''
            )}
  ...
  return (
    <LayoutOrderManagement
      dataLang={dataLang}
      titleHead={id ? dataLang?.returns_title_edit || 'returns_title_edit' : dataLang?.returns_title_child || 'returns_title_child'}
      breadcrumbItems={breadcrumbItems}
      titleLayout={id ? dataLang?.returns_title_edit || 'returns_title_edit' : dataLang?.returns_title_child || 'returns_title_child'}
      searchBar={
          ...
          {listData.length === 0 ? (
            ...
            <div>
              ...
              <div className='grid grid-cols-12 gap-3 2xl:gap-4 items-center sticky top-0 py-2 z-10 border-b border-gray-100'>
                <TableHeader className='text-left col-span-3'>{dataLang?.import_from_items || 'import_from_items'}</TableHeader>
                <div className='col-span-9'>
                  <div className='grid grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.6fr)_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,1.2fr)_minmax(0,0.2fr)] gap-3 2xl:gap-4'>
                    ...
                    <TableHeader className='text-center'>{dataLang?.import_from_quantity || 'import_from_quantity'}</TableHeader>
                    <TableHeader className='text-center'>{dataLang?.import_from_unit_price || 'import_from_unit_price'}</TableHeader>
                    {/* Chọn hoàng loạt % chiết khấu */}
                    <DropdownDiscount value={discount} onChange={_HandleChangeInput.bind(this, 'discount')} dataLang={dataLang} />
                    <TableHeader className='text-center'>{dataLang?.returns_sck || 'returns_sck'}</TableHeader>
                    {/* Chọn hàng loại % Thuế */}
                    <DropdownTax value={tax} onChange={_HandleChangeInput.bind(this, 'tax')} dataLang={dataLang} taxOptions={taxOptions} />
                  ...
                  {isFetching ? (
                    ...
                    <>
                      {listData?.map((e, index) => {
                        ...
                        return (
                          <div key={e?.id?.toString()} className={`grid items-center grid-cols-12 gap-3 2xl:gap-4 my-1 ${isLast ? '' : 'border-b border-[#F3F3F4]'}`}>
                            {/* Mặt hàng */}
                            ...
                            <div className='col-span-9'>
                                  ...
                                  e?.child?.map(ce => (
                                    <React.Fragment key={ce?.id?.toString()}>
                                      ...
                                      <div className='flex flex-col justify-center'>
                                        <div className='flex flex-col w-full'>
                                          <SelectCustomLabel
                                            ...
                                            isError={(errWarehouse && ce?.kho == null) || (errWarehouse && (ce?.kho?.label == null || ce?.kho?.warehouse_name == null))}
                                            renderOption={option => {
                                              return (
                                                (option?.warehouse_name || option?.label || option?.qty) && (
                                                  <div>
                                                    <div className='flex gap-1 3xl:text-[12px] 2xl:text-[10px] xl:text-[9.5px] text-[9px] leading-normal font-semibold'>
                                                      <h2>
                                                        {dataLang?.returns_wareshoue || 'returns_wareshoue'}: {option?.warehouse_name}
                                                      </h2>
                                                    </div>
                                                    <div className='flex gap-1 3xl:text-[11px] 2xl:text-[10px] xl:text-[9.5px] text-[9px] leading-normal font-medium'>
                                                      <h2>
                                                        {dataLang?.returns_wareshouePosition || 'returns_wareshouePosition'}: {option?.label}
                                                      </h2>
                                                    </div>
                                                    <div className='flex gap-1 2xl:text-[11px] xl:text-[9.5px] text-[9px] leading-normal font-medium'>
                                                      <h2>
                                                        {dataLang?.returns_survive || 'returns_survive'}: {formatNumber(option?.qty)}
                                                      </h2>
                                                    </div>
                                                  </div>
                                                )
                                          ...
                                          />
                                          {((errWarehouse && ce?.kho == null) || (errWarehouse && (ce?.kho?.label == null || ce?.kho?.warehouse_name == null))) && (
                                      ...
                                      <div
                                        ...
                                        } ${ce?.amount === null || ce?.amount === '' || ce?.amount === 0 ? 'border-red-500 hover:border-red-500 focus:border-red-500' : ''}`}
                                      >
                                        <button
                                          disabled={ce?.amount === 1 || ce?.amount === '' || ce?.amount === null || ce?.amount === 0}
                                      ...
                                      <ItemTotalAndDelete
                                        total={formatMoney(ce?.price * (1 - Number(ce?.discount) / 100) * (1 + Number(ce?.tax?.tax_rate) / 100) * Number(ce?.amount))}
                              ...
                              {/* TODO: Show thuộc tính kho không nhập form */}
                            </div>
                          </div>
                        );
                      })}
      ...
      note={
        <div>
          <div className='responsive-text-base font-normal text-secondary-color-text mb-3 capitalize'>{dataLang?.returns_reason || 'returns_reason'}</div>
          <textarea
            value={note}
