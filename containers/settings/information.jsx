            ...
            <div className='bg-[#F3F4F6] p-4 rounded grid grid-cols-3 gap-5'>
              ...
              <div className='flex flex-col justify-between h-full col-span-2'>
                ...
                <div className='w-full h-2.5 bg-white rounded-full relative'>
                  <div className='absolute left-0 bg-gradient-to-r from-[#1556D9] to-[#8FE8FA] h-2.5 rounded-full' style={{ width: `${dataSetting?.completion_percent}%` }} />
                </div>
                  ...
                  <div className='flex flex-col gap-1'>
                    ...
                    <div className='flex divide-x divide-red-500'>
                      {dataSetting?.missing_fields?.map((e, i) => (
                        <h6 key={i} className={`${i === 0 ? 'pl-0' : ''} text-[#EE1E1E] font-[300] text-[14px] px-2 w-fit`}>
                          {e}
                        </h6>
                      ))}
                    </div>
  ...
  return (
    <React.Fragment>
      ...
      <div>
        ...
        <Btn_Setting url='/settings/print-template' isActive='/settings/print-template'>
          Thiết lập mẫu in
        </Btn_Setting>
...
const Btn_Setting = React.memo(props => {
  ...
  return (
    <Link href={props.url ? props.url : '#'} alt={props.children}>
      <button
        className={`${
          router.asPath.includes(props.isActive) ? 'text-white bg-[#11315B]' : 'text-[#11315B] hover:bg-[#11315B]/5'
