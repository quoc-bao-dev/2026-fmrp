...
const Popup_dskh = props => {
  ...
  // tự động chọn chi nhánh mặc định khi tạo mới
  useEffect(() => {
    ...
    <>
      ...
      >
        <div className='flex items-center space-x-4 my-3 border-[#E7EAEE] border-opacity-70 border-b-[1px]'>
          <button onClick={_HandleSelectTab.bind(this, 0)} className={`${tab === 0 ? 'text-[#0F4F9E]  border-b-2 border-[#0F4F9E]' : 'hover:text-[#0F4F9E] '}  px-4 py-2 outline-none font-semibold`}>
            {props.dataLang?.client_popup_general || 'client_popup_general'}
          </button>
          <button onClick={() => _HandleSelectTab(1)} className={`${tab === 1 ? 'text-[#0F4F9E]  border-b-2 border-[#0F4F9E]' : 'hover:text-[#0F4F9E] '}  px-4 py-2 outline-none font-semibold`}>
            {props.dataLang?.client_popup_contact || 'client_popup_contact'}
          </button>
          <button onClick={_HandleSelectTab.bind(this, 2)} className={`${tab === 2 ? 'text-[#0F4F9E]  border-b-2 border-[#0F4F9E]' : 'hover:text-[#0F4F9E] '}  px-4 py-2 outline-none font-semibold`}>
          ...
          <form onSubmit={_HandleSubmit.bind(this)} className=''>
            {tab === 0 && (
              <Customscrollbar className='3xl:h-[600px]  2xl:h-[470px] xl:h-[380px] lg:h-[350px] h-[400px]'>
                <Form dataLang={props.dataLang} dataGroup={dataGroup} dataWard={dataWard} dataDitrict={dataDitrict} isState={isState} queryState={queryState} dataChar={dataChar} />
              </Customscrollbar>
            )}
            {tab === 1 && (
              <div>
                <Customscrollbar className='3xl:h-[600px]  2xl:h-[470px] xl:h-[380px] lg:h-[350px] h-[400px]'>
                  <div className='flex flex-wrap justify-between p-2 space-x-1'>
                    {isState.option.map(e => (
                      <FormContactInfo key={e.idFe?.toString()} option={e} dataLang={props.dataLang} onChangOptions={onChangOptions} onDelete={handleQueryId}>
                ...
                <Customscrollbar className='3xl:h-[600px]  2xl:h-[470px] xl:h-[380px] lg:h-[350px] h-[400px] overflow-y-auto'>
                  <div className='flex flex-wrap justify-between p-2 space-x-1 '>
                    {isState.optionDelivery.map(e => (
                      <FormContactDelivery key={e.idFe?.toString()} optionDelivery={e} dataLang={props.dataLang} onChangOptionsDelivery={onChangOptionsDelivery} onDelete={handleQueryId}>
                        <IconDelete className='transition duration-200 group-hover:text-white animate-bounce ease-linea' />
                      </FormContactDelivery>
                    ))}
