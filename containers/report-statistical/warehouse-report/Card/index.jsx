...
const Card = props => {
    ...
    const displayedData = useMemo(() => {
        const baseData = Array.isArray(dataReportStock?.data) ? dataReportStock.data : [];
    ...
    }, [dataReportStock?.data, dataReportStock?.rTotal]);
                ...
                tableSection={
                    <TableSection
                        fixedColumns={[
                            ...
                            { title: 'Ngày duyệt kho', width: 'w-44', textAlign: 'left' },
                            { title: 'Ngày chứng từ', width: 'w-44', textAlign: 'left' },
                        ...
                        renderFixedRow={(item, index) => (
                            <>
                                <RowItemTable
                                    className={`w-14 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}
                                >
                                    {item._rowType ? '' : index}
                                </RowItemTable>
                                <RowItemTable className={`w-44 flex flex-col justify-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}>
                                    <span>{item._rowType ? (item._rowType === 'opening' ? 'Tồn đầu kỳ' : 'Tồn cuối kỳ') : item.item_code}</span>
                                </RowItemTable>
                                <RowItemTable className={`w-44 flex items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}>
                                    <div className='flex flex-col gap-1 justify-start'>
                                        {item._rowType ? (
                                            <p className='text-left responsive-text-sm text-neutral-07 font-semibold'>-</p>
                                        ) : (
                                            <>
                                                <p className='text-left responsive-text-sm text-neutral-07 font-normal'>{item.item_name}</p>
                                                <p className='text-left responsive-text-xxs text-neutral-03 font-normal'>{item.item_variation || ''}</p>
                                            </>
                                        )}
                                    </div>
                                </RowItemTable>
                        ...
                        renderScrollableRow={(item, index) => (
                            <>
                                {/* Đơn vị tính */}
                                <RowItemTable
                                    className={`w-80 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}
                                >
                                    {item._rowType ? '-' : item.unit_name}
                                </RowItemTable>

                                {/* Tồn đầu Kỳ */}
                                <RowItemTable
                                    className={`w-48 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}
                                >
                                    {Number(item.opening_qty) === 0 ? '-' : formatNumber(Number(Math.abs(item.opening_qty || 0)))}
                                </RowItemTable>
                                <RowItemTable
                                    className={`w-36 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}
                                >
                                    {formatMoneyOrDash(Number(item.opening_value || 0))}
                                </RowItemTable>

                                {/* Nhập Kho */}
                                <RowItemTable className={`w-40 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] !text-new-blue font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}>
                                    {item._rowType ? (
                                        '-'
                                ...
                                <RowItemTable
                                    className={`w-40 flex justify-center items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 font-normal flex-shrink-0 ${getSpecialRowBg(item)}`}
                                >
                                    {item._rowType ? '-' : formatMoneyOrDash(Number(item.in_value || 0))}
                                </RowItemTable>
                            </>
                        )}
                    />
