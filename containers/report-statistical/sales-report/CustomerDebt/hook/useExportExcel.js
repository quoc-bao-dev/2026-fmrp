...
export const exportCustomerDebtExcel = (dataCustomerDebt, filename = 'Bao_cao_cong_no_khach_hang.xlsx') => {
  ...
  // Opening balance
  rows.push(['Công nợ đầu kỳ', '', '', '', '', '', '', '', '', Number(dataCustomerDebt?.debt?.debt_begin) || 0]);

  // Deliveries section
  if (Array.isArray(dataCustomerDebt?.deliveries) && dataCustomerDebt.deliveries.length > 0) {
    rows.push(['Phiếu giao hàng', '', '', '', '', '', '', '', '', '']);
    ...
    // Deliveries total
    rows.push(['Tổng cộng (giao hàng)', '', '', '', '', '', Number(dataCustomerDebt?.total_deliveries) || 0, '', '', Number(dataCustomerDebt?.total_deliveries) || 0]);
  }

  // Returns section
  if (Array.isArray(dataCustomerDebt?.returns) && dataCustomerDebt.returns.length > 0) {
    rows.push(['Trả lại hàng bán', '', '', '', '', '', '', '', '', '']);
    ...
    rows.push(['Tổng cộng (trả lại)', '', '', '', '', '', Number(dataCustomerDebt?.total_returns) || 0, '', '', Number(dataCustomerDebt?.total_returns) || 0]);
  }

  // Other payslips coupon (Phiếu thu)
  if (Array.isArray(dataCustomerDebt?.other_payslips_coupon) && dataCustomerDebt.other_payslips_coupon.length > 0) {
    rows.push(['Phiếu thu trong kỳ', '', '', '', '', '', '', '', '', '']);
    ...
    rows.push(['Tổng cộng (phiếu thu)', '', '', '', '', '', '', '', '', Number(dataCustomerDebt?.total_other_payslips_coupon) || 0]);
  }

  // Final balance
  rows.push(['Tổng cộng phát sinh trong kỳ', '', '', '', '', '', '', '', '', (Number(dataCustomerDebt?.total_deliveries) || 0) - (Number(dataCustomerDebt?.total_returns) || 0)]);
  rows.push(['Số dư cuối kỳ', '', '', '', '', '', '', '', '', Number(dataCustomerDebt?.final_debt) || 0]);
  ...
  // Apply borders and basic alignment
  const range = XLSX.utils.decode_range(ws['!ref']);
  ...
  // Section header coloring and merges
  ws['!merges'] = ws['!merges'] || [];
  ...
  // Find and color section rows by label
  for (let R = 1; R <= range.e.r; R++) {
    const firstRef = XLSX.utils.encode_cell({ r: R, c: 0 });
    const val = ws[firstRef]?.v;
    if (val === 'Phiếu giao hàng') colorRow(R, 'DBEAFE'); // blue-100
    if (val === 'Trả lại hàng bán') colorRow(R, 'FEF3C7'); // amber-100
    if (val === 'Phiếu thu trong kỳ') colorRow(R, 'D1FAE5'); // emerald-100
  }
  ...
  // Column widths
  ws['!cols'] = headers.map((_, idx) => ({ wch: [16, 18, 34, 12, 12, 14, 16, 14, 12, 18][idx] || 14 }));
