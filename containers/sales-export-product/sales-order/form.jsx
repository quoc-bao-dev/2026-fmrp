...
const SalesOrderForm = props => {
  ...
  useEffect(() => {
    if (dataBranch.length > 0 && dataBranch[0]?.value) {
      if (authState.branch?.length > 0 && !selectedBranch) {
        ...
        skipBranchResetRef.current = true;
  ...
  useEffect(() => {
    setFlagStateChange(selectedBranch === undefined);
  ...
  // Map text_type to dataKey for TagColorProduct
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      setSelectedPersonalContact(rResult?.contact_id !== null && rResult?.contact_id !== '0' ? rResult?.contact_id : null);
  ...
  const handleSubmitValidate = e => {
    ...
    if (typeOrder === '0') {
      if (startDate === null || selectedCustomer === null || selectedBranch === null || selectedStaff == null || deliveryDate === null || deliveryDateInOption === true) {
      ...
      if (startDate === null || selectedCustomer === null || selectedBranch === null || selectedStaff == null || deliveryDate === null || deliveryDateInOption === true || quote === null) {
        startDate === null && setErrDate(true);
        selectedCustomer === null && sErrCustomer(true);
        selectedBranch === null && setErrBranch(true);
        selectedStaff == null && setErrStaff(true);
  ...
  const handleSubmit = async () => {
    ...
    formData.append('staff_id', selectedStaff ?? staff?.value ?? '');
    ...
    if (isTotalMoney?.totalPrice >= 0 && isTotalMoney?.totalDiscountPrice >= 0 && isTotalMoney?.totalDiscountAfterPrice >= 0 && isTotalMoney?.totalTax >= 0 && isTotalMoney?.totalAmount >= 0) {
      try {
        ...
        if (isSuccess) {
          ...
          isShow('error', dataLang && dataLang[message] ? dataLang[message] : message);
        }
        setOnSending(false);
      } catch (error) {}
    } else {
      isShow('error', newDataOption?.length === 0 ? `Chưa chọn thông tin mặt hàng!` : 'Có lỗi trong thông tin mặt hàng, vui lòng kiểm tra lại!');
      setOnSending(false);
    }
  };
  ...
  const breadcrumbItems = [
    ...
    {
      label: `${id ? dataLang?.sales_product_edit_order || 'sales_product_edit_order' : dataLang?.sales_product_add_order || 'sales_product_add_order'}`,
    },
  ];
  ...
  const selectItemsLabel = option => (
    <div className='flex p-2 hover:bg-gray-100 rounded-md cursor-pointer items-center justify-between font-deca'>
      <div className='flex gap-3 items-start w-[calc(100%-80px)]'>
        <img src={option.e?.images ?? '/icon/noimagelogo.png'} alt={option.e?.name} className='xl:size-16 size-12 object-cover rounded-md' />
      ...
      <div className='text-red-500 responsive-text-sm min-w-[80px] text-right whitespace-nowrap'>{formatNumber(option.e?.price_sell || option.e?.price)} đ</div>
    </div>
  );
  ...
  return (
    <LayoutOrderManagement
      dataLang={dataLang}
      titleHead={id ? dataLang?.sales_product_edit_order || 'sales_product_edit_order' : dataLang?.sales_product_add_order || 'sales_product_add_order'}
      breadcrumbItems={breadcrumbItems}
      titleLayout={id ? dataLang?.sales_product_edit_order || 'sales_product_edit_order' : dataLang?.sales_product_add_order || 'sales_product_add_order'}
      searchBar={
          ...
          {option.length <= 0 ? (
            ...
            <div>
              ...
              <div className='grid items-center grid-cols-[minmax(0,2.2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,0.6fr)_minmax(0,1fr)_minmax(0,0.8fr)_minmax(0,1.2fr)_minmax(0,0.2fr)] 2xl:gap-x-6 gap-x-4 sticky top-0 py-2 border-b border-gray-100'>
                ...
                <TableHeader className='text-center'>{dataLang?.sales_product_quantity || 'sales_product_quantity'}</TableHeader>
                <TableHeader className='text-center'>{dataLang?.sales_product_unit_price || 'sales_product_unit_price'}</TableHeader>
                {/* Chọn hoàng loạt % chiết khấu */}
                <DropdownDiscount value={totalDiscount} onChange={handleOnChangeInput.bind(this, 'totaldiscount')} dataLang={dataLang} />
                <TableHeader className='text-center'>{dataLang?.returns_sck || 'returns_sck'}</TableHeader>
                {/* Chọn hàng loại % Thuế */}
                <DropdownTax value={totalTax} onChange={value => handleOnChangeInput('total_tax', value)} dataLang={dataLang} taxOptions={taxOptions} />
                <TableHeader className='text-right'>{dataLang?.sales_product_total_into_money || 'sales_product_total_into_money'}</TableHeader>
              </div>
                      ...
                      >
                          ...
                          <div className='flex items-center justify-between gap-1 xl:gap-2'>
                            <div className='flex items-start'>
                              <div className='flex xl:flex-row flex-col items-start gap-3'>
                                <img src={e?.item?.e?.images ?? '/icon/noimagelogo.png'} alt={e?.item?.e?.name} className='2xl:size-16 size-12 object-cover rounded-md' />
                                <div className='flex flex-col gap-[2px] responsive-text-xxs overflow-hidden text-neutral-03 font-normal'>
                                  <h3 className='font-semibold responsive-text-sm text-brand-color'>{e?.item?.e?.name}</h3>
                                  <p>
                                    {/* Màu sắc: <span> */}
                        ...
                        <div className='flex items-center justify-center'>
                          <div
                            className={`flex items-center justify-center h-8 2xl:h-10 3xl:p-2 xl:p-[2px] p-[1px] rounded-3xl cursor-text border ${
                              e?.quantity <= 0 || e?.quantity == '' ? 'border-red-500' : 'focus:border-brand-color hover:border-brand-color border-neutral-N400'
                            }`}
                          >
                            <button
                        ...
                        <div
                          className={`flex items-center rounded-lg h-8 2xl:h-10 py-1 px-2 2xl:px-3 border ${
                            e?.price === '' || (e?.price === null ? 'border-red-500' : 'focus:border-brand-color hover:border-brand-color border-neutral-N400')
                          }`}
                        >
                          <InPutMoneyFormat
                            value={e?.price}
        ...
        <div className='flex flex-col gap-4 relative'>
          ...
          <div className='flex flex-col flex-wrap items-center gap-y-3 relative'>
            ...
            <div className='w-full'>
              ...
              {errDeliveryDate && <label className='text-sm text-red-500'>{'Vui lòng chọn ngày cần hàng'}</label>}
            </div>
          </div>
          {/* Khách hàng */}
          <SelectWithRadio
            ...
            value={selectedCustomer ?? customer?.value}
                    ...
                    <div className='w-full'>
                      <div className='relative flex flex-row select-with-sort'>
                        ...
                        <SelectWithSort
                          ...
                          value={selectedStaff ?? staff?.value}
                  ...
                  <div className='flex flex-col flex-wrap items-center mb-4 gap-y-3'>
                    ...
                    <div className='w-full relative flex select-with-sort'>
                      ...
                      <SelectWithSort
                        ...
                        value={selectedPersonalContact ?? contactPerson?.value}
      ...
      note={
        <div className='w-full mx-auto'>
          <h4 className='responsive-text-base font-normal text-secondary-color-text mb-3 capitalize'>{dataLang?.sales_product_note || 'sales_product_note'}</h4>
