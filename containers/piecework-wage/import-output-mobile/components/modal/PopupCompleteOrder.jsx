...
const ProductRow = memo(({ product, index, updateProductQuantity, updateProductError, handleSelectProduct, errorTags, errorImages, onAddTag, onRemoveTag, onAddImage, onRemoveImage }) => {
  ...
  return (
    <div className='flex flex-col gap-2 bg-white p-2 rounded-lg'>
      <div className='flex gap-2.5 cursor-pointer' onClick={handleToggleRowSelect}>
        <Image src={product.images || '/icon/default/default.png'} alt={product.item_name} width={200} height={200} className='size-[64px] flex-shrink-0 object-cover rounded' />
        <div className='flex flex-col gap-2 flex-1'>
          ...
          <div
            ...
            onTouchStart={e => e.stopPropagation()}
          >
            <span className='text-10 font-normal text-[#667085] whitespace-nowrap'>SL đạt</span>
            <InputNumberCustom
              state={product.quantity_success}
              setState={value => updateProductQuantity(index, value)}
              allowDecimal={true}
              skipBlurSetState={true}
              classNameButton='flex-shrink-0 size-6 bg-white'
              classNameInput='w-full text-xs'
            />
            <span className='text-10 font-normal text-[#667085] whitespace-nowrap'>SL lỗi</span>
            <InputNumberCustom
              state={errorValue}
      ...
      <AnimatePresence>
        {hasError && (
          <motion.div
            ...
            className='bg-gray-50 p-2 rounded-lg flex flex-col gap-3 w-full'
          >
            {/* Tags Section */}
...
const PopupCompleteOrder = ({ stage_id, stage_name, po, isOpen, onClose, is_production_input, timesheet_id, start_date, end_date, is_product, end_timer }) => {
  ...
  const updateProductQuantity = useCallback(
    async (index, value) => {
      setProducts(prevProducts => {
        ...
        // Nếu sản phẩm đang CHƯA được chọn mà người dùng nhập số, mới "nhảy lên đầu".
        ...
        // Chỉ khi trước đó chưa chọn -> đưa lên đầu
        if (!product?.selected) {
  ...
  const handleSelectProduct = useCallback((index, checked) => {
    setProducts(prevProducts => {
      ...
      if (checked) {
        // Chỉ khi trước đó chưa chọn -> đưa lên đầu.
        // Nếu đã chọn sẵn, giữ nguyên vị trí.
        if (!product?.selected) {
          updatedProducts.splice(index, 1);
          updatedProducts.unshift(updatedProduct);
        } else {
        ...
        // Bỏ chọn: đưa xuống cuối danh sách
        updatedProducts.splice(index, 1);
  ...
  return (
    ...
    >
      <div className='p-3 flex flex-col gap-3 w-full h-full bg-[#F3F4F6] font-deca'>
        ...
        <Customscrollbar className='flex-1 min-h-0 max-h-full'>
        ...
        <button onClick={handleConfirm} className='whitespace-nowrap flex items-center justify-center gap-2 text-lg font-normal rounded-lg p-2 w-full text-white bg-blue-fmrp hover:opacity-80'>
