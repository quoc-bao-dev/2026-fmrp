...
/*
  ...
  Props:
    ...
    - onCompletedChange: (completed: boolean) => void (optional) -> fires on completion status change
*/
const OtpInput = ({ length = 6, onComplete, onChange, onCompletedChange }) => {
    ...
    useEffect(() => {
        const completed = values.every(v => v !== '');
        if (typeof onCompletedChange === 'function') onCompletedChange(completed);
        if (completed) {
            const code = values.join('');
            if (typeof onComplete === 'function') onComplete(code);
        }
    }, [values, onComplete, onCompletedChange]);
    ...
    const boxClass =
        'w-[68px] h-[68px] text-center rounded-[12px] border border-[#E4E7EC] outline-none flex items-center justify-center text-[40px] font-semibold text-[#11315B] focus:outline-none focus:border-[#0F4F9E] focus:ring-2 transition-all';
