...
const TableSection = ({
  ...
  useEffect(() => {
    const checkInitialScroll = () => {
      if (scrollbarRef.current) {
        try {
          const scrollableEl = scrollbarRef.current.getScrollElement ? scrollbarRef.current.getScrollElement() : scrollbarRef.current.querySelector('.simplebar-content-wrapper');
  ...
  const hasFixedColumns = useFixedLayout && fixedColumns.length > 0;

  // Render multi-level headers
  const renderMultiLevelHeaders = () => {
    if (!groupedHeaders && !hasFixedColumns) {
      return (
        <div className='z-30 bg-white border-x border-t border-[#E0E0E1]'>
          <div className='flex bg-white border-b border-[#E0E0E1] w-full'>
            {scrollableColumns.map((column, index) => (
              <ColumnTable
                key={index}
                className={`flex items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 !responsive-text-sm font-semibold capitalize ${column.width || 'flex-1 min-w-0'} ${
                  column.textAlign === 'center' ? 'justify-center' : column.textAlign === 'end' ? 'justify-end' : 'justify-start'
                }`}
              >
                {column.title}
              </ColumnTable>
    ...
    if (!groupedHeaders) {
      ...
      return (
        <div className='flex z-30 bg-white border-x border-t border-[#E0E0E1]'>
          ...
          {hasFixedColumns && (
            ...
            >
              {fixedColumns.map((column, index) => (
                <ColumnTable
                  key={index}
                  className={`flex items-center py-2 px-3 border-r border-[#E0E0E1] text-neutral-07 !responsive-text-sm font-semibold flex-shrink-0 capitalize ${column.width} ${
                    column.textAlign === 'center' ? 'justify-center' : column.textAlign === 'end' ? 'justify-end' : 'justify-start'
                  }`}
                >
                  {column.title}
          ...
          <div
            className={`flex bg-white border-b border-[#E0E0E1] w-full ${hasFixedColumns ? 'overflow-auto no-scrollbar' : ''}`}
            ref={hasFixedColumns ? headerScrollRef : null}
            style={hasFixedColumns ? { overflowY: 'hidden' } : undefined}
            onScroll={hasFixedColumns ? handleHeaderScroll : undefined}
          >
            {scrollableColumns.map((column, index) => (
              <ColumnTable
                key={index}
                ...
                }  items-center text-neutral-07 !responsive-text-sm font-semibold ${hasFixedColumns ? 'flex-shrink-0' : 'flex-1 min-w-0'} ${column.width} ${
                  column.textAlign === 'center' ? 'justify-center' : column.textAlign === 'end' ? 'justify-end' : 'justify-start'
                }`}
              >
                {column.title}
    ...
    return (
      <div className='z-30 bg-white border-x border-t border-[#E0E0E1]'>
        ...
        <div className='flex'>
          ...
          <div className='flex bg-white overflow-auto no-scrollbar w-full' ref={headerScrollRef} style={{ overflowY: 'hidden' }} onScroll={handleHeaderScroll}>
            ...
            {scrollableColumns &&
              ...
              scrollableColumns.map((column, index) => (
                <div
                  key={`single-${index}`}
                  className={`flex items-center justify-center py-2 px-3 border-r border-b border-[#E0E0E1] text-neutral-07 !responsive-text-sm font-semibold bg-white flex-shrink-0 capitalize ${
                    column.width
  ...
  return (
    <div className='h-full relative overflow-hidden flex flex-col'>
      ...
      <Customscrollbar className='h-full flex-1 overflow-auto border-x border-b border-[#E0E0E1]' onScroll={handleContentScroll} ref={scrollbarRef} alwaysShowScrollbar={true}>
        <div className={`relative h-full ${useFixedLayout ? 'min-w-full w-max' : 'w-full'}`}>
          {isFetching ? (
      ...
      {renderFooter ? (
        <div className='flex sticky bottom-0 z-10 bg-white w-full'>
          <div className='flex overflow-auto no-scrollbar w-full' ref={footerScrollRef} style={{ overflowY: 'hidden' }} onScroll={handleFooterScroll}>
            {renderFooter()}
          </div>
        </div>
      ) : (
