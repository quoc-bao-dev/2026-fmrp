...
const PopupConfim = props => {
  ...
  return (
    <Popup open={props.isOpen} closeOnDocumentClick={false} onClose={props.onClose} className={`${props.className} popup-edit`} overlayStyle={{ zIndex: 1100 }}>
      <div
        className={`min-w-[400px] max-w-[400px] ${
          props.nameModel == 'price_quote_status' && 'min-w-[500px]'
      ...
      >
        <div className={`${inter.className} bg-[#ffffff] p-4 shadow-xl rounded-xl flex flex-col gap-3`}>
          <div className='relative inline-block'>
            {props.type == 'warning' ? <AlertTriangleIcon className='text-yellow-500 size-6' /> : <Image alt='teddd' src='/popup/check-circle.png' width={24} height={24} className='object-cover' />}
          </div>
          <h1 className='text-[#101828] font-medium 3xl:text-[22px] 2xl:text-[18px] text-lg'>{props.title}</h1>
          <h2 className='text-[#667085] font-medium responsive-text-lg tracking-widest-[0.14px]	'>{props.subtitle}</h2>
