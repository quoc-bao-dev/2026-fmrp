...
const NoData = ({ type = 'table', className = '', classNameImage = '3xl:max-w-[280px] max-w-[200px] w-full h-auto object-contain', classNameTitle = 'text-sm', titleText = '', ...rest }) => {
  ...
  const title = isTable ? titleText || 'Chưa có dữ liệu' : isComment ? 'Hãy thảo luận trao đổi tình hình sản xuất ngay!' : isReport ? titleText || 'Chưa có dữ liệu' : 'Không tìm thấy các mục';
  ...
  return (
    ...
    >
      <div className='h-full flex flex-col justify-center items-center 3xl:gap-5 gap-3 3xl:py-5 py-3 mx-auto'>
        <Image src={imageSrc} width={1000} height={1000} alt='nodata' className={classNameImage} priority />
        <h1 className={`${isTable ? `3xl:text-2xl xl:text-base text-lg text-[#52575E] ${classNameTitle}` : `${classNameTitle} text-[#141522] opacity-90`} font-medium`}>{title}</h1>
      </div>
    </motion.div>
  );
};
