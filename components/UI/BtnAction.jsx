...
import { printReturnsPDF } from '@/containers/purchase-order/returns/utils/printReturnsPDF';
...
import { fetchPDFPurchaseOrder, fetchPDFPurchaseOrderImport } from '@/managers/api/purchase-order/useLinkFilePDF';
...
import { fetchPDFWarehouseTransfer } from '@/managers/api/warehouse-transfer/useLinkFilePDF';
...
const Popup_Pdf = props => {
  ...
  const handlePrintTem = async ({ typePrint, id, typePage }) => {
    ...
    // In FE cho returns (phiếu trả hàng NCC) (có giá / không giá)
    if (typePage === 'returns') {
      try {
        const response = await apiReturns.apiDetailPageReturns(id);
        const returnsData = response?.data || response?.result || response;
        ...
        console.error('Lỗi khi in PDF returns:', error);
        isShow('error', `Lỗi khi in phiếu: ${error.message || 'Không xác định'}`);
      } finally {
        setIsLoadingPrint(false);
        setLoadingOption(null);
    ...
    <div className='relative' ref={dropdownRef}>
      ...
      {isOpen && (
        <div className='absolute top-full -right-5 p-1 mt-1 w-fit bg-white rounded-xl z-[999] border border-gray-200 shadow-[0px_20px_40px_-4px_#919EAB3D,0px_0px_2px_0px_#919EAB3D]'>
          <div className=''>
            {props.props?.type === 'import' || props.props?.type === 'deliveryReceipt' || props.props?.type === 'returns' ? (
              <PopupPrintItem
                dataLang={props.dataLang}
...
export const BtnAction = React.memo(props => {
  ...
  useEffect(() => {
    const handleClickOutside = event => {
      ...
      const isInsidePopup = event.target.closest('.popup-edit') || event.target.closest('.popup-content') || event.target.closest('.popup-overlay') || event.target.closest('.reactjs-popup');
  ...
  const fetchPDFMultiplePage = {
    ...
    warehouseTransfer: fetchPDFWarehouseTransfer,
  };
  ...
  const handleClick = () => {
    ...
    if (!!props?.id && props?.type === 'import') {
      if (props?.warehouseman_id != '0' || props?.status_pay != 'not_spent') {
        isShow(
          'error',
          `${(props?.warehouseman_id != '0' && props.dataLang?.warehouse_confirmed_cant_edit) || (props?.status_pay != 'not_spent' && (props.dataLang?.paid_cant_edit || 'paid_cant_edit'))}`
        );
        return;
      }
      handleQueryPage();
    ...
    const checkType = ['production_warehouse', 'productsWarehouse', 'recall', 'exportToOther', 'warehouseTransfer', 'returns', 'deliveryReceipt', 'returnSales', 'servicev_voucher'];
  ...
  const renderActionButtons = () => {
    ...
    if (props.type == 'order') {
      ...
      allButtons.push(
        <Popup_dspc key='payment' onRefresh={props.onRefresh} dataLang={props.dataLang} id={props?.id} className='text-sm hover:bg-slate-50 text-left cursor-pointer'>
          {props.dataLang?.purchase_order_table_edit || 'purchase_order_table_edit'}
        </Popup_dspc>
      );
    } else if (!['order', 'products', 'servicev_voucher', 'receipts', 'payment', 'recall', 'production_warehouse'].includes(props.type)) {
      const totalButtons = calculateTotalButtons();
      ...
      allButtons.push(
        <button
          ...
          className={`group rounded-lg w-full p-1 border border-transparent transition-all ease-in-out flex items-center gap-2 responsive-text-sm text-left cursor-pointer
                                ${totalButtons > 3 ? 'hover:bg-primary-05' : 'hover:border-[#064E3B] hover:bg-[#064E3B]/10'}`}
        >
          <EditIcon
            className={`size-5 transition-all duration-300 
                            ${totalButtons > 3 ? 'text-neutral-03 group-hover:text-neutral-07' : ''}`}
          />
          {totalButtons > 3 && <p className='text-neutral-03 group-hover:text-neutral-07 font-normal whitespace-nowrap'>Sửa phiếu</p>}
        </button>
      );
    }
    ...
    if (props.type == 'sales_product') {
      if (role == true || auth?.orders?.is_create == 1 || auth?.orders?.is_edit == 1) {
        ...
        allButtons.push(
          ...
          >
            <BoxSearch size={20} className='group-hover:text-amber-500 group-hover:shadow-md' />
            <p className='group-hover:text-amber-500 pr-2.5'>{props.dataLang?.salesOrder_see_stock_keeping || 'salesOrder_see_stock_keeping'}</p>
          </button>
        );
      }
    }
            ...
            <div className='absolute top-full -right-5 p-1 mt-1 w-fit bg-white rounded-xl z-[999] border border-gray-200 shadow-[0px_20px_40px_-4px_#919EAB3D,0px_0px_2px_0px_#919EAB3D]'>
              <ul className='flex flex-col gap-1' data-row-id={currentId}>
                <li
                  onClick={() => {
                    dispatch({
                      ...
                      payload: {
                        open: true,
                        children: <PopupPrintTemNVL id={currentId} />,
                      },
                    });
                    setPrintDropdownOpen(false);
                  }}
                ...
                >
                  ...
                  <p className='whitespace-nowrap'>{props?.dataLang?.btn_table_print_notprice || 'In không giá'}</p>
                </li>
                <li
                  onClick={() => {
                ...
                >
                  <PrinterTemIcon className='size-5' />
                  <p className='whitespace-nowrap'>{props?.dataLang?.btn_table_print_price || 'In có giá'}</p>
                </li>
              </ul>
            </div>
          )}
    ...
    if (props.type == 'order') {
      ...
      allButtons.push(
        <button
          ...
          className={`group rounded-lg w-full p-1 border border-transparent transition-all ease-in-out flex items-center gap-2 responsive-text-sm text-left cursor-pointer
                        ${totalButtons > 3 ? 'hover:bg-primary-05' : 'hover:border-red-01 hover:bg-red-02'}`}
          {...(totalButtons <= 3 && {
        ...
        >
          ...
          />
          {totalButtons > 3 && <p className='text-neutral-03 group-hover:text-neutral-07 font-normal whitespace-nowrap'>{props.dataLang?.btn_table_delete || 'btn_table_delete'}</p>}
        </button>
      );
    }
    ...
    <div className='flex items-center justify-center gap-1'>
      ...
      {props.type === 'products' && (
        <PopupConfim
          ...
          subtitle={
            <span>
              Bạn phải <span className='font-bold capitalize text-new-blue'>thiết kế công đoạn</span> cho thành phẩm này trước khi thiết kế BOM. Bạn có muốn mở thiết kế công đoạn không?
            </span>
          }
